function [sac, micro] = FindAllSaccadesInTrial(eye_pos, graphics, noise_elim, monk)
...
v_thresh = 10; % vel thresh in degrees per second
v_sample_thresh = 10; % vel must stay above thresh for at least this many ms
d_thresh = 0.1+monk/2; % displacement thresh in degrees
min_between = 50; % can't have two sacs/micros within this many ms
micro_cutoff = 1; % degrees.  Under this is a micro, over is a regular sac


eye_pos(:,1) = smooth(eye_pos(:,1),5,'gauss');
eye_pos(:,2) = smooth(eye_pos(:,2),5,'gauss');
n_micro = 0;
n_reg = 0;
micro = struct([]);
sac = struct([]);
...
elseif strcmp(kernel_type,'halfgauss')
    x = [-window-1:window+1];
    kernel = exp(-x.^2/(window/2)^2);
    kernel = kernel(1:window+1);
else
